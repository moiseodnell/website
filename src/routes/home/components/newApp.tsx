...
export const NewApp: React.FC<any> = ({}) => {
  ...
  const {newapp, newapp_screens, appstore, playstore} = ImagesFont;
  ...
  return (
    ...
    <div className="relative w-2/3 xl:w-5/12 sm:w-2/3 lg:w-5/12 mb-8 lg:mb-0">
        ...
        <img src={newapp_screens} alt="Mobile App Screens" className="relative mx-auto lg:mx-0 lg:mr-8" />
    </div>
    ...
    <div className="text-center xl:w-1/3 lg:w-1/3 lg:text-left md:mx-10">
        ...
        <a href="https://blog.payqin.com" className="inline-block bg-payqinBlueNew text-white font-semibold py-3 px-8 rounded-lg hover:bg-blue-600 mb-12">{t("index.learnMore")}</a>
        ...
        <div className="flex justify-center md:justify-center lg:justify-start flex-row xl:flex-row gap-x-4 xl:flex lg:flex md:flex sm:flex lg:flex-row sm:flex-row md:flex-row">
          <a href="https://apps.apple.com/fr/app/payqin/id1397872810"><img src={appstore} alt="App Store" className="h-12"/></a>
          <a href="https://play.google.com/store/search?q=payqin&c=apps"><img src={playstore} alt="Google Play" className="h-12"/></a>
        </div>
    </div>
</div>
  );
